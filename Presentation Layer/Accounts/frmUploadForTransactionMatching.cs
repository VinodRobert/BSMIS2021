...
namespace BSMIS 
{
    public partial class frmUploadForTransactionMatching : Form
    {
        DataTable _transactionLists;

        public frmUploadForTransactionMatching()
        {
            InitializeComponent();
        }
        ...
        public DataTable GetDataTableFromExcel(string path, bool hasHeader = true)
        {
            ...
            using (var pck = new OfficeOpenXml.ExcelPackage())
            {
                ...
                if (ws.Name.ToUpper() != "TRANSID")
                {
                    MessageBox.Show("Worksheet Name Not Proper - Must be TRANSID", "Excel File Read Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1);
                    _errorExcelFormat = 1;
                    tbl = null;
                    return tbl;
                }
                ...
                if (_errorExcelFormat == 0)
                {
                    ...
                    if (tbl.Columns[0].Caption.ToUpper() != "BATCHID")
                    {
                        MessageBox.Show("Column Name 1 Not Proper - Must Be BATCHID", "Excel File Read Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1);
                        _errorExcelFormat = 1;
                    }


                    if (tbl.Columns[1].Caption.ToUpper() != "TRANSID")
                    {
                        MessageBox.Show("Column Name 2 Not Proper - Must Be TRANSID", "Excel File Read Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1);
                        _errorExcelFormat = 1;
                    }
        ...
        private void btnRead_Click(object sender, EventArgs e)
        {
            ...
            if (firstWorksheet.Name.ToUpper() != "TRANSID")
            {
                MessageBox.Show("First WorkSheet Not Proper. The Label Must be TRANSID ", "Excel File Read", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1);
            }
        ...
        public int ValidateImportedData(DataTable _dtSales)
        {
            ...
            for (_row = 0; _row <= _maxRowCount - 1; _row++)
            {
                ...
                {
                    _loadingErrorMessage = _loadingErrorMessage + "  BATCHID ID  Cannot Be Null or Empty ";
                ...
                {
                    _loadingErrorMessage = _loadingErrorMessage + "  TRANSID  Cannot Be Null or Empty ";
                    MessageBox.Show(_loadingErrorMessage, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    _totalErrorCount = _totalErrorCount + 1;
                    _errorInData = 0;
                ...
                _currentID = Convert.ToString(_transIDROW[0]);
                if (!IsDigitsOnly(_currentID))
                {
                    _loadingErrorMessage = _loadingErrorMessage + "  BATCHID Must be  a Number ";
                    MessageBox.Show(_loadingErrorMessage, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    _totalErrorCount = _totalErrorCount + 1;
                    _errorInData = 0;
                }
                ...
                if (!IsDigitsOnly(_currentID))
                {
                    _loadingErrorMessage = _loadingErrorMessage + "  TRANSID  Must be  a Number ";
                    MessageBox.Show(_loadingErrorMessage, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    _totalErrorCount = _totalErrorCount + 1;
                    _errorInData = 0;
                }
        ...
        private void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            DoMatching();
            MessageBox.Show("Success!!!!  All Transactions Matched ");
            this.Close();
        }
        ...
        private void EmptyMatchingTable()
        {
            ...
            _sql = "DELETE FROM BS.TRANSMATCHING";
        ...
        private void DoMatching()
        {
            ...
            int result;
            string _connectionString = SqlHelper.GetConnectionString();
            _sql = "EXEC BS.spMatchingTransactions";
            result = SqlHelper.ExecuteNonQuery(_connectionString, CommandType.Text, _sql);
        }
        ...
        public void SaveToTables(DataTable transTable)
        {
            ...
            Int32 batchID;
            Int32 transID;
            DataTable _matchingTable = new DataTable("TRANSMATCHING");
            _matchingTable.Columns.Add(new DataColumn("BATCHID", typeof(Int32)));
            _matchingTable.Columns.Add(new DataColumn("TRANSID", typeof(Int32)));

            for (int i = 0; i <= _maxRowCount - 1; i++)
            {
                _tranIDRow = transTable.Rows[i];
                batchID = Convert.ToInt32(_tranIDRow[0]);
                transID = Convert.ToInt32(_tranIDRow[1]);
                _matchingTable.Rows.Add(batchID, transID);
            }
            string _connectionString = SqlHelper.GetConnectionString();
            using (SqlBulkCopy bulkCopy = new SqlBulkCopy(_connectionString))
            {
                bulkCopy.BulkCopyTimeout = 600; // in seconds
                bulkCopy.DestinationTableName = "BS.TRANSMATCHING";
                bulkCopy.WriteToServer(_matchingTable);
            }
        }
