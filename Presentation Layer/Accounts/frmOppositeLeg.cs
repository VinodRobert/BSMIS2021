...
namespace BSMIS 
{
    public partial class frmOppositeLeg : Form
    {
        int _CurrentFinYear;
        int _CurrentMonth;
        ...
        private void frmCreditorAge_Load(object sender, EventArgs e)
        {
            ...
            lblHeader.Text = "Ledger Report (Opposite Legs)";
            ActivateCurrentFinYear();
            ResetAll();
            LoadCombos();
            GetAllLedgerCodeLedgerName();
            LoadListZone();
            LoadListProject();
        }
       ...
       //     string _query = "SELECT LEDGERCODE,UPPER(LEDGERNAME) LEDGERNAME FROM LEDGERCODES WHERE LEDGERSUMMARY=0 AND LEDGERALLOC IN ('Contracts') AND LEDGERSTATUS=0 AND LEDGERNAME<>'' ORDER BY LEDGERNAME  ";
            ...
            UtilityFunctions.LoadWindowsCombo(cmbLedgerCode, dtLedgerCompleteList, "LEDGERNAME", "LEDGERCODE", 0);
        }
        ...
        private void GenerateResult()
        {
            ...
            _ledgerCode = Convert.ToString(cmbLedgerCode.SelectedValue).ToString();
           
            _dsFinalResult = new DataSet();
            _dt = new DataTable();
            _dsFinalResult = _cg.LedgerWithOppositeLegs(_finYear, _fromPeriod, _toPeriod, _tableProjects, _ledgerCode);
        ...
        private void btnFilter_Click(object sender, EventArgs e)
        {
            ...
            {
                filter = " LedgerName like '" + filter + "%'";
            }
                dsLedgerCompleteList.Tables[0].DefaultView.RowFilter = filter;
                dtLedgerCompleteList = (dsLedgerCompleteList.Tables[0].DefaultView).ToTable();
