...
namespace BSMIS
{
    public class CreditorAge
    {
        ...
        {
            string _sql = "SELECT DISTINCT [YEAR] FINYEAR FROM TRANSACTIONS WHERE YEAR>=2016 AND YEAR<= " + Convert.ToString(_currentFinYear)+"  ORDER BY YEAR ";
            string _connectionString = SqlHelper.GetConnectionString();
        ...
        {
            string _connectionString = SqlHelper.GetConnectionString();
            string _sql = "SELECT  BORGID, BORGNAME FROM BORGS   ORDER BY BORGNAME   ";
            DataSet ds = SqlHelper.ExecuteDataset(_connectionString, CommandType.Text, _sql);
            return ds;
        }
        ...
        public DataSet OrgXTAB(int _year, int _reportType, int _fromPeriod, int _toPeriod, DataTable _Ledgers, DataTable _OrgIDS)
        {
            ...
            string _connectionString = SqlHelper.GetConnectionString();
            DataSet ds = SqlHelper.ExecuteDataset(_connectionString, CommandType.StoredProcedure, "BI.spORGXTABS", arParms);
            return ds;
        ...
        public DataSet PRRegister(string _fromDate, string _toDate, DataTable _Creditors, DataTable _OrgIDS)
        {
            SqlParameter[] arParms = new SqlParameter[4];
            // @ZONE is the Input Parameter
